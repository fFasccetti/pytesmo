...
from pytesmo.validation_framework.data_manager_Ini import DataManager
...
class Validation(object):
    ...
    def __init__(self, datasets, temporal_matcher, metrics_calculator,
                 ...
                 scale_to_other=False, cell_based_jobs=True,triple=1,matching_together=1):
        """
        ...
        self.triple = triple
        self.matching_together = matching_together
    def calc(self, job):
        """
        Takes either a cell or a gpi_info tuple and performs the validation.
        ...
        for gpi_info in zip(process_gpis, process_lons, process_lats):
            ...
            if self.triple == 1:
                ...
                for other in other_dataframes.keys():
                    list_system.append(other_dataframes[other])

                joined = self.temp_matching(ref_dataframe,*list_system)
                ...
                if self.scaling is not None:
                    try:
                        joined = scaling.scale(joined, method=self.scaling, reference_index=self.scale_to_index)
                    except ValueError:
                        continue
                # for the metrics the columns are evaluated between the columns with the name of ref other1 other2
                ...
                #result_tc.append(self.calc_metrics(joined,gpi_meta))
                results[('TC','result')].append(self.calc_metrics(joined,gpi_meta))
            elif self.triple == 0:

                if self.matching_together==0:
                    joined_data = {}
                    for other in other_dataframes.keys():
                    ...
                    joined = self.temp_matching(ref_dataframe,*list_system)
                    # joined is pandasDataFrames of all the dataset collocated in time, the name of columns are the 'sm' field for each sensors
                    # do not change the structure of the code below and to create the couple ref other (but with the same sample number)
                    for other in other_dataframes.keys():
                        joined_data[other] = joined[[ref_dataframe.keys()[0],other_dataframes[other].keys()[0]]] #0 is the sm
        ...
        #    results[('TC','results')] = []
        #    results[('TC','results')]= result_tc

        compact_results = {}
        if self.triple==1:
            if len(results[results.keys()[0]])>0:
